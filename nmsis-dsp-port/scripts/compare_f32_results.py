...
LOG_ROOT = Path('/tmp/nmsis-f32-result-compare')
SIM_LOG_DIR = LOG_ROOT / 'sim'
HOST_LOG_DIR = LOG_ROOT / 'host'
HOST_BIN_DIR = LOG_ROOT / 'host-bin'
RESULT_CSV = ROOT / 'manifests/f32_result_compare.csv'
RESULT_MD = ROOT / 'manifests/F32_RESULT_COMPARE.md'
...
def main():
    ...
    for row in rows:
        if instrument_case(row):
            changed += 1
    print(f'instrumented {changed} files; total f32 PASS cases {len(rows)}')
    results = []
