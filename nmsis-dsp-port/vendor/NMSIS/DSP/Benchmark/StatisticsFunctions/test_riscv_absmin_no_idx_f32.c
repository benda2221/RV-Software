...
void absminNoIdx_riscv_absmin_no_idx_f32(void)
{
    ...
    __zr_hash = zircon_result_hash_combine(__zr_hash, absmin_no_idx_f32_input, (uint32_t)sizeof(absmin_no_idx_f32_input));
    __zr_hash = zircon_result_hash_combine(__zr_hash, &f32_output, (uint32_t)sizeof(f32_output));
    printf("@@RESULT@@ case=absminNoIdx_riscv_absmin_no_idx_f32 hash=0x%08x\n", (unsigned int)__zr_hash);
}