...
BENCH_START(riscv_mat_scale_f32);
    ...
    __zr_hash = zircon_result_hash_combine(__zr_hash, mat_scale_f32_array, (uint32_t)sizeof(mat_scale_f32_array));
    __zr_hash = zircon_result_hash_combine(__zr_hash, &result, (uint32_t)sizeof(result));
    printf("@@RESULT@@ case=matScale_riscv_mat_scale_f32 hash=0x%08x\n", (unsigned int)__zr_hash);
