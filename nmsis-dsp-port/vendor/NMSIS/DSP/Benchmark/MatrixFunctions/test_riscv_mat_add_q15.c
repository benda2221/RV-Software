...
void matAdd_riscv_mat_add_q15(void)
{
    ...
    __zr_hash = zircon_result_hash_combine(__zr_hash, mat_add_q15_output, (uint32_t)sizeof(mat_add_q15_output));
    __zr_hash = zircon_result_hash_combine(__zr_hash, &result, (uint32_t)sizeof(result));
    printf("@@RESULT@@ case=matAdd_riscv_mat_add_q15 hash=0x%08x\n", (unsigned int)__zr_hash);
